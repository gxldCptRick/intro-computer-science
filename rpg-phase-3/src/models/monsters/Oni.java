...
public class Oni extends Monster implements Droppable {

	public Oni(int str, int dex, int intel) {

		super(str, dex, intel, 25, 3, 1, 3, "Oni");

		int randomAttr = rnJesus.nextInt(wAttrs.length - 1) + 1;
		WeaponAttribute newAttr = wAttrs[randomAttr];
		this.storeWeapon(new Weapon(WeaponType.GLAIVE, newAttr));
	...
	public Armor generateArmorDrop() {
		...
		if (chance > 74) {
			...
			newArmor = new Armor(ArmorType.CHAINMAIL, aAttrs[randomAttr]);
